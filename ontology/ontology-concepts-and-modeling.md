...
> **两阶段抽取**：先用**规则 / 词典**把 CVE 编号、IP、域名这类“长得规范”的东西摘出来——几乎零成本、可复现、不会幻觉；剩下的自然语言再交给 **LLM**，按冻结好的本体抽实体与关系——灵活、高召回，但会幻觉，输出必须落在 Schema 内并过质量门禁（§2.4 SHACL）。一句话：**规则保精度，LLM 保召回。**
...
3. **与 LLM 的分工**：抽取走“规则 → LLM”两阶段（见 §1.5），LLM 另负责**起草**本体；OWL/SHACL 负责**约束与校验**——LLM 输出必须落在 Schema 内、过 SHACL 门禁。抽取阶段规则先行是为了**省算力**，校验阶段规则兜底是为了**保精度**，两者互为补充。
